...
from gemini_gen import model_choices, test_gemini_key
...
class WebdocRuntime:
    ...
    def test_gemini(self) -> Dict[str, Any]:
        ...
        model = str(self.settings.get("gemini_model") or "gemini-2.5-flash").strip()
    ...
    def _publish(self, kws: List[str]) -> List[str]:
        ...
        folder = os.path.join(out, f"webdoc_gukjeinfo_{stamp}")
        root = project_root()
        ...
        gemini_model = str(self.settings.get("gemini_model") or "gemini-2.5-flash").strip()
        if gen_mode == "gemini":
